...
class MsppCardKm extends StatelessWidget {
  @override
...
class ExpandedMeetData extends StatelessWidget {
  ...
  Future<bool> resultTextField({int index}) {
    return Get.defaultDialog(
        ...
        confirmTextColor: Colors.black87,
        onConfirm: () {
          controller.textFieldPI('km')[index] =
  ...
  TextButton buildTextButtonRemark(int id) {
    return TextButton(
      ...
      child: Obx(() => Text(controller.textFieldKM[id] == ""
          ? 'Klik disini'
          : "${controller.textFieldKM[id]}")),
    );
  }
  ...
  TextButton buildTextButtonAssessment(int id) {
    return TextButton(
      ...
      child: Obx(() => Text(controller.radioIndexKM[id] == -1
          ? 'Pilih disini'
          : '${controller.radioData[controller.radioIndexKM[id]]}')),
    );
  }

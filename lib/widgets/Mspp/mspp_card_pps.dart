...
class MsppCardPps extends StatelessWidget {
  @override
...
class ExpandedMeetData extends StatelessWidget {
  ...
  Future<bool> resultTextField({int index}) {
    return Get.defaultDialog(
        ...
        confirmTextColor: Colors.black87,
        onConfirm: () {
          controller.textFieldPI('pps')[index] =
  ...
  TextButton buildTextButtonRemark(int id) {
    return TextButton(
      ...
      child: Obx(() => Text(controller.textFieldPPS[id] == ""
          ? 'Klik disini'
          : "${controller.textFieldPPS[id]}")),
    );
  }
  ...
  TextButton buildTextButtonAssessment(int id) {
    return TextButton(
      ...
      child: Obx(() => Text(controller.radioIndexPPS[id] == -1
          ? 'Pilih disini'
          : '${controller.radioData[controller.radioIndexPPS[id]]}')),
    );
  }
